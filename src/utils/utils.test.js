...
describe('extractHeightFromString', () => {
  ...
  test('if an invalid string is passed, return null', () => {
    ...
    // Test that we are not picking up the 'm' in 'something'
    expect(utils.extractHeightFromString('something')).toEqual(null);
  });

  test(`if a valid string is passed, it should try to decipher the height value 
    and return the value and unit`, () => {
    ...
    expect(utils.extractHeightFromString('6ft 0in'))
        .toEqual([{value: 6, unit: 'ft'}, {value: 0, unit: 'in'}]);
...
describe('extractWeightFromString', () => {
  ...
  test('if an invalid string is passed, return null', () => {
    ...
    // Test that we are not picking up the 's' in 'something'
    expect(utils.extractWeightFromString('something')).toEqual(null);
  });
  ...
  test(`if a valid string is passed, it should try to decipher the height value 
    ...
    expect(utils.extractWeightFromString('1.2 lbs 5kgs'))
        .toEqual([{value: 1.2, unit: 'lb'}, {value: 5, unit: 'kg'}]);
  });
});
...
describe('isValidMeasurementArray', () => {
  ...
  test('if the parameter is an array of valid objects, return true', () => {
    ...
    expect(utils.isValidMeasurementArray([{value: 1, unit: 'cm', someIrrelevantKey: 'something'}]))
        .toEqual(true);
...
describe('createAbsoluteUrl', () => {
  ...
  test('if invalid parameters are passed, throw error', () => {
    expect(() => {
      utils.createAbsoluteUrl(1, 1);
    }).toThrow(Error);
    expect(() => {
      utils.createAbsoluteUrl({}, {});
    }).toThrow(Error);
    expect(() => {
      utils.createAbsoluteUrl([], []);
    }).toThrow(Error);
  ...
  test(`if valid strings are passed, it should prepend the site root URL 
      if it does not already exist in the given string`, () => {
    expect(utils.createAbsoluteUrl('https://www.site.com/', '/some/directory/file.png'))
        .toEqual('https://www.site.com/some/directory/file.png');
    expect(utils.createAbsoluteUrl('https://www.site.com/', 'some/directory/file.png'))
        .toEqual('https://www.site.com/some/directory/file.png');
    expect(utils.createAbsoluteUrl('https://www.site.com', '/some/directory/file.png'))
        .toEqual('https://www.site.com/some/directory/file.png');
  ...
  test(`if valid strings are passed, it should not prepend the site root URL 
      if it already exists in the given string`, () => {
    expect(utils.createAbsoluteUrl('https://www.site.com/', 'https://www.site.com/some/directory/file.png'))
        .toEqual('https://www.site.com/some/directory/file.png');
    expect(utils.createAbsoluteUrl('https://www.site.com', 'https://www.site.com/some/directory/file.png'))
        .toEqual('https://www.site.com/some/directory/file.png');
  });
